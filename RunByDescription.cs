...
class RunByDescription : IRun
{
    private static RunByDescription _instance = new();
    private string? _filePath;

    private RunByDescription() { }

    public static RunByDescription GetInstance()
    {
        return _instance;
    }
    ...
    public void Run()
    {
        try
        {
            ...
            var query = from line in contents
                        where !string.IsNullOrEmpty(line) && !new string[] { "#", "!", ">" }.Where(str => line.StartsWith(str)).Select(str => str).Any()
                        select line;
            ...
            while (true)
            {
                ...
                for (int i = 0; i < vocabularies.Count; i++)
                {
                    ...
                    if (vo.description == string.Empty || Regex.Match(vo.description, @"^\((?:(?:(?:expressions?)|(?:r?uv)|(?:i|t)|(?:it)|(?:(?:n|i)a)|(?:suru)|(?:adv)|(?:noun)|(?:grammar)(?:suffix)(?:prefix))(?:,\s)?)+?\)$").Success)
                    {
